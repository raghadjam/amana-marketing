...
// Dynamically import BubbleMap to avoid SSR issues
const BubbleMap = dynamic(() => import('../../src/components/ui/bubble-map').then(mod => mod.BubbleMap), { ssr: false });
...
export default function RegionView() {
  ...
  const aggregatedData = useMemo((): RegionViewResult => {
    ...
    if (!campaigns.length) return initialRegionResult;
    ...
    const averageRoas = finalRegionalList.length ? finalRegionalList.reduce((sum, item) => sum + item.roas, 0) / finalRegionalList.length : 0;
    const topRegion = finalRegionalList.sort((a, b) => b.revenue - a.revenue)[0]?.region || 'N/A';

    const revenueMapData = finalRegionalList.map(item => ({
      region: item.region,
    ...
    const spendMapData = finalRegionalList.map(item => ({
      region: item.region,
      country: item.country,
      value: item.spend,
      color: getCoordinates(item.region).color || '#94a3b8',
    ...
    <div className="flex flex-col lg:flex-row min-h-screen bg-gray-900">
      ...
      <div className="flex-1 flex flex-col transition-all duration-300 ease-in-out overflow-hidden">
        ...
        <div className="flex-1 p-6 overflow-y-auto w-full max-w-full">
          {/* Metrics Cards section could go here */}
